...
def main():
    ...
    if training_args.do_predict:
        ...
        np.save(os.path.join(training_args.output_dir,
                             "test_results.npy"), predictions)
        predictions = np.argmax(predictions, axis=1)

        sub = pd.read_csv('data/new_test.csv')
        candidates = sub[["ending0", "ending1", "ending2", "ending3"]].values.tolist()
        output_test_file = os.path.join(training_args.output_dir, "test_results.csv")
        if trainer.is_world_process_zero():
